...
export default function MapView({ center, markers = [], polylines = [], level = 5 }: MapViewProps) {
  ...
  const overlaysRef = useRef<kakao.maps.CustomOverlay[]>([]); // ✅ CustomOverlay 객체를 저장할 Ref
  ...
  useEffect(() => {
    return () => {
      overlaysRef.current.forEach(o => o.setMap(null)); // ✅ CustomOverlay 제거
  ...
  useEffect(() => {
    ...
    markers.forEach((markerData) => {
      ...
      const overlay = new kakao.maps.CustomOverlay({
        ...
        position: position,
        content: content,
        yAnchor: 1.6, // 핀 이미지와 라벨을 함께 고려한 yAnchor 값
      });
  ...
  useEffect(() => {
    ...
    polylines.forEach((polylineData) => {
      ...
      const linePath = polylineData.path.map(coord => new kakao.maps.LatLng(coord.lat, coord.lng));
